...
function applyAuthUI() {
  ...
  var ch = document.getElementById("loginChoice");
  ...
  if (!hint) return;
  if (net.account) {
    hint.textContent = "Xin chào " + net.account.name + ". Hồ sơ đã đồng bộ.";
    var nma = document.getElementById("homeName");
    if (nma) nma.classList.remove("guest-name");
  }
  else if (net.guest) {
  ...
  else hint.textContent = "Hãy đăng nhập hoặc vào với tư cách khách để chơi.";
}
function mustLogin() {
  if (signedIn()) return true;
  applyAuthUI();
  document.getElementById("homeHint").textContent = "Cần đăng nhập (email hoặc khách) trước khi chơi.";
  document.getElementById("loginChoice").classList.add("show");
  return false;
}
function sendHello() {
...
function renderFind(list) {
  ...
  box.innerHTML = "";
  if (!list.length) { box.textContent = "Không thấy ai online."; return; }
  list.forEach(function (u) {
    var row = document.createElement("div");
    row.className = "find-row";
    row.innerHTML = "<span>" + u.name + (u.busy ? " · đang đấu" : u.room ? " · trong bàn" : " · rảnh") + "</span>";
    var b = document.createElement("button");
    b.textContent = "Mời";
    b.disabled = !!u.busy;
...
(function initHome() {
  ...
  if (hm) hm.onclick = function () {
    ...
    if (homeMusicOn) startHomeMusic(); else stopHomeMusic();
  };
  ...
  lockNameField();
  function openAuth(kind, text) {
    pendingAuth = kind;
    document.getElementById("authTitle").textContent = kind === "register" ? "Đăng ký" : kind === "login" ? "Đăng nhập" : "Quên mật khẩu";
    document.getElementById("authText").textContent = text;
    document.getElementById("authOtp").style.display = "none";
    document.getElementById("authNewPass").style.display = "none";
    document.getElementById("authPop").classList.add("show");
  }
  document.getElementById("btnRegister").onclick = function () {
    document.getElementById("loginChoice").classList.remove("show");
    document.getElementById("idBox").classList.add("show");
    document.getElementById("accPass2").style.display = "inline-block";
  ...
  document.getElementById("btnLoginMail").onclick = function () {
    document.getElementById("loginChoice").classList.remove("show");
    document.getElementById("idBox").classList.add("show");
    document.getElementById("accPass2").style.display = "none";
    pendingAuth = "login";
  };
  ...
  document.getElementById("btnAuthGo").onclick = function () {
    openAuth(pendingAuth === "register" ? "register" : "login",
      pendingAuth === "register" ? "Đồng ý tạo tài khoản và lưu thành tích?" : "Đồng ý đăng nhập tài khoản này?");
  };
  document.getElementById("btnAuthCancel").onclick = function () {
    document.getElementById("idBox").classList.remove("show");
    pendingAuth = null;
  };
  document.getElementById("btnLogout").onclick = function () {
    openAuth("logout", "Đồng ý đăng xuất? Cần đăng nhập lại mới được chơi.");
  };
  function saveRemember() {
    var on = document.getElementById("chkRemember").checked;
    try {
      if (!on) localStorage.removeItem("coupCred");
      else localStorage.setItem("coupCred", JSON.stringify({
        contact: document.getElementById("accContact").value,
        pass: document.getElementById("accPass").value
      }));
    } catch (e) {}
  }
  document.getElementById("chkRemember").onchange = saveRemember;
  try {
    var cred = JSON.parse(localStorage.getItem("coupCred") || "null");
    if (cred && cred.contact) {
      document.getElementById("accContact").value = cred.contact;
      document.getElementById("accPass").value = cred.pass || "";
      document.getElementById("chkRemember").checked = true;
    }
  } catch (e) {}
  document.getElementById("btnEye").onclick = function () {
    var p1 = document.getElementById("accPass");
    var p2 = document.getElementById("accPass2");
  ...
  document.getElementById("btnForgot").onclick = function () {
    openAuth("forgot", "Hệ thống sẽ gửi OTP về email / SĐT / Facebook đã đăng ký.");
  };
  ...
  document.getElementById("btnAuthYes").onclick = function () {
    var contact = document.getElementById("accContact").value;
    var pass = document.getElementById("accPass").value;
    var name = document.getElementById("homeName").value;
    var via = document.getElementById("accVia").value;
    connectNet(function () {
      if (pendingAuth === "register") {
        var pass2 = document.getElementById("accPass2").value;
        if (pass !== pass2) { addLog("Mật khẩu nhập lại không khớp."); return; }
        saveRemember();
        netSend({ type: "register", name: name, contact: contact, via: via, pass: pass });
      } else if (pendingAuth === "login") {
        saveRemember();
        netSend({ type: "login", contact: contact, pass: pass });
      }
      else if (pendingAuth === "logout") {
        net.account = null;
        net.guest = false;
        netSend({ type: "logout" });
      ...
      else if (pendingAuth === "forgot") netSend({ type: "forgot", contact: contact });
      else if (pendingAuth && pendingAuth.kind === "reset") {
        netSend({
          type: "reset",
          contact: pendingAuth.contact,
          otp: document.getElementById("authOtp").value,
          pass: document.getElementById("authNewPass").value
        });
      }
      document.getElementById("authPop").classList.remove("show");
    });
  ...
  document.getElementById("btnHero").onclick = function () {
    if (!mustLogin()) return;
    goTable();
    connectNet(function () {
      sendHello();
