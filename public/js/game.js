...
function layout() {
  const side = 128;
  const byW = window.innerWidth - side - 36;
  const byH = (window.innerHeight - 70) * 640 / 720;
  const maxW = Math.max(280, Math.min(520, byW, byH));
  canvas.width = maxW;
...
function resetBoard() {
  ...
  function place(color, flipR) {
    ...
    bag.forEach((trueType, i) => {
      ...
      board[flipR(sl.r)][sl.c] = {color, type: trueType, revealed:false, slot: sl.t};
    });
  }
  place("red", r => r);
  place("black", r => 9 - r);
...
function applyMove(mv, fromNet) {
  ...
  if (net.online && !fromNet) relay({kind:"move", mv: mv});
}
...
function sendChat(txt) {
  ...
  showChat(who, txt);
  document.getElementById("chatWrap").classList.remove("open");
  if (net.online) relay({kind:"chat", who: who, text: txt});
}
(function buildQuickChat() {
  const box = document.getElementById("chatPop");
...
document.getElementById("btnAgain").onclick = backToRoom;
