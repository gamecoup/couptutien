...
function findOrLinkOAuth(info) {
  ...
  if (!acc) {
    ...
    acc.provider = acc.provider || info.provider;
    acc.providerId = acc.providerId || info.providerId;
    acc.via = acc.via || info.provider;
    if (info.email) acc.email = acc.email || info.email;
...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === "/auth/google/callback" || urlPath === "/auth/facebook/callback") {
    ...
    const done = function (ok, need) {
      redirect(res, "/oauth-done.html?ok=" + (ok ? "1" : "0") + "&need=" + (need ? "1" : "0"));
    };
    ...
    run.then(function (info) {
      ...
      const need = !acc.nameSet || !acc.name;
  ...
  if (urlPath === "/oauth-done.html") {
    htmlPage(res, "Đăng nhập",
      "<script>try{if(window.opener)window.opener.postMessage({type:'oauth',ok:location.search.indexOf('ok=1')>=0,need:location.search.indexOf('need=1')>=0},location.origin);}catch(e){}setTimeout(function(){location.href='/?oauth='+(location.search.indexOf('ok=1')>=0?'ok':'fail');},300);</script><p>Đang quay lại game...</p>");
    return;
  }
...
function assignColorsAndJoin(room, ws) {
  ...
  ws.roomId = room.id;
  ws.spectate = false;
  send(ws, { type: "joined", room: room.id, color: color, count: room.players.length });
  seat(room);
  broadcastList();
}
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "create") {
      ...
      const room = {
        ...
        busy: false,
        specs: [],
        players: [{ ws, host: true, color: null }]
      };
      rooms.set(id, room);
      ws.roomId = id;
      send(ws, { type: "created", room: id, password: !!password });
      broadcastList();
      return;
    }

    if (msg.type === "play") {
      const target = findWaiting(ws);
      if (!target) {
        send(ws, { type: "error", text: "Chưa có bàn nào đang chờ. Hãy tạo phòng hoặc đợi người khác." });
    ...
    if (msg.type === "join") {
      ...
      if (already) {
        ws.roomId = room.id;
        send(ws, { type: "joined", room: room.id, color: already.color, count: room.players.length });
        seat(room);
        return;
      }
      ...
      if (room.players.length >= 2) {
        if (msg.spectate) {
          ...
          ws.roomId = room.id;
          ws.spectate = true;
          send(ws, { type: "spectate", room: room.id, specs: specCount(room) });
    ...
    if (msg.type === "begin") {
      ...
      if (!host || !host.ready) {
        send(ws, { type: "error", text: "Chủ phòng chưa sẵn sàng." });
        return;
      }
      br.busy = true;
      br.over = false;
    ...
    if (msg.type === "oauth-claim") {
      const rec = oauth.takeClaim(ws.token) || oauth.takeClaim(String(msg.sid || ""));
    ...
    if (msg.type === "search") {
      ...
      wss.clients.forEach((c) => {
        ...
        const n = norm(c.profile.name);
        const ct = c.account ? c.account.contact : "";
        if (q && n.indexOf(q) < 0 && ct.indexOf(q) < 0) return;
        list.push({
          id: c.profile.id,
          name: c.profile.name,
          room: c.roomId || null,
      ...
      send(ws, { type: "search", list: list.slice(0, 20) });
      return;
    }
    ...
    if (msg.type === "invite-ok") {
      ...
      rooms.set(id, room);
      from.roomId = id;
      send(from, { type: "created", room: id, password: false });
      assignColorsAndJoin(room, ws);
      return;
    }
