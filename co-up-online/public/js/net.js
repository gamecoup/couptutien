...
function onNetMsg(ev) {
  ...
  if (msg.type === "account") {
    ...
    net.guest = false;
    applyAuthUI();
    if (msg.needProfile || (msg.acc && !msg.acc.nameSet)) openProfilePop();
...
(function initHome() {
  ...
  function openOAuth(kind) {
    connectNet(function () {
      ...
      var w = window.open(url, "oauth", "width=520,height=640");
  ...
  window.addEventListener("message", function (ev) {
    ...
    connectNet(function () {
      sendHello();
      netSend({ type: "oauth-claim", sid: sessionStorage.getItem("coupSess") || "" });
    });
  });
